...
## for testing
# from mipego.utils import boundary_handling
# from mipego.SearchSpace import ContinuousSpace, OrdinalSpace, NominalSpace
...
class mies(object):
    def __init__(self, search_space, obj_func, var_class, x0=None, max_eval=np.inf,
                 minimize=True, mu_=4, lambda_=28, sigma0=None, eta0=None, P0=None,
                 verbose=False, ):
        #TODO: xopt and fopt depend on the setting of mu and lambda, how to set mu and lambda?
        ...
        self.obj_func = obj_func # expect obj_func is dictionary
        ...
        if x0 is not None:                         # given x0
        ...
        else: # in the case of mipego, x0=None
            x = np.asarray(self._space.sampling(self.mu_), dtype='object')  # uniform sampling
            #TODO: let x store different values, instead of such case that x = [11,16,11,11]
            par = np.tile(sigma0, (self.mu_, 1))
            if eta0 is not None:
                par = np.c_[par, np.tile(eta0, (self.mu_, 1))]
            if P0 is not None:
                ##### typo: self.N_i --> self.N_p, we know self.N_p=1
                par = np.c_[par, np.tile([P0] * self.N_p, (self.mu_, 1))]
            # Note: None type should not be assigned the position in individual vector
            ###### delete None type placehoder, useful when e.g. no continuous variable
            mask = par != np.array(None)
            par = par[mask].reshape(self.mu_, -1)

            # evaluate mu individuals and initialize xop and fopt
            ...
            b = np.nonzero(self.f_mu == self.fopt)[0]
            a = int(b[0])  # must convert np.int64 to int
            self.xopt = self.pop_mu[a][self._id_var]
            # a = np.nonzero(self.f_mu == self.fopt)[0][0]
    ...
    def _extract_and_evaluate(self, alg, var):
        valid_id = np.nonzero(self.var_class == alg)[0]
        v = [var[int(id)] for id in valid_id] # implement v = var[valid_id]
        v_f = self.obj_func[alg](v)
        return v_f
    ...
    def evaluate(self, pop):
        ...
        for i, individual in enumerate(pop):
            ...
            ## check var[0] and evaluate part of var, let v_f be the fitness of entire var
            for alg in self.alg_name:
                if var[0] == alg:
                    f[i] = self._extract_and_evaluate(alg=alg, var=var)
    ...
    def stop(self):
        ...
        return any(self.stop_dict.values())# any(list): there is one True in list, return any
...
if __name__ == '__main__':
    ...
    def fitness1(x):
        ...
        x_r, x_i, x_d = np.array([x[0], x[1]]), x[2], x[3]
    ...
    if a == 1:
        ...
        opt = mies(space, fitness1, max_eval=1e3, verbose=False)
    ...
    if c == 22:
        def fitness2(x):
            ...
            if b == 2:
                ...
                opt = mies(space, fitness2, max_eval=500, minimize=False, verbose=True)
                xopt, fopt, stop_dict = opt.optimize()
                # print(stop_dict)
                ...
                fopt = np.zeros((1, N))
                for i in range(N):
                    opt = mies(space, fitness2, max_eval=500, verbose=False)
