...
#        self.bounds = self._space.bounds
        ...
        self.stop_dict = {}
        self._check_params()
        # record fitness value of each incumbent and corresponding function evaluation count
        self.hist_f = []
        self.hist_iteration = []
        self.hist_evaluation = []
        self.hist_x = []
        self.hist_incumbent = []
...
#        copyreg.pickle(types.MethodType, self.obj_func)
    ...
    def _eval_one(self, x, runs=1):
        ...
        ans = [self.obj_func(x.to_dict()) for i in range(runs)] # give obj_func() real arguments 实际参数
        # pass (x, gpu, bounds, var_names) to obj_func function object
        fitness = np.sum(ans)
        ...
        if isinstance(data, Solution):
            ...
            if self.n_jobs > 1:
                ...
                for i, x in enumerate(data): # data = [sample1, sample2, sample3]
                    x, runs, hist, hist_id = self._eval_one(x)
#                    data[i] = x # maybe not able to run when n_point=1
                    self.eval_count += runs
                    self.eval_hist += hist
                    self.eval_hist_id += hist_id
                    self.hist_x.append(x.to_array())
    ...
    def fit_and_assess(self, surrogate = None):
        while True:
            try:
                ...
                # add an if statement here to make sure fitness_scaled valid, 
                # because sometimes, e.g. for KNN, all values in fitness array are equal
...
#        self.logger.info('Surrogate model r2: {}'.format(r2))
    ...
    def gpuworker(self, q, gpu_no):
        ...
        while True:
            ...
            self.hist_iteration.append(self.iter_count)
            self.hist_evaluation.append(self.eval_count)
    ...
    def step(self):
        if not hasattr(self, 'data'):
            self._initialize()
            ##### record best fitness in DoE
            init_fitness = [s.fitness for s in self.data]
            bestF_DoE = np.min(np.array(init_fitness))
            self.hist_f.append(bestF_DoE)
        ...
        self.hist_iteration.append(self.iter_count)
        self.hist_evaluation.append(self.eval_count)
        self.hist_incumbent.append(self.incumbent.to_array())
