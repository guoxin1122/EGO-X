...
class InfillCriteria:
    ...
    def __init__(self, model, plugin=None, minimize=True):
        ...
        if self.plugin is None:
            ...
            # surrogate_model.y is fitness_scaled = (fitness - _min) / (_max - _min)
            # self.plugin is present best solution
    @abstractmethod
    def __call__(self, X):
...
class EI(InfillCriteria):
    ...
    def __call__(self, X, dx=False):
        ...
        if sd < 1e-6:
            ...
            return f_value
        ## Note: the following if statement always False, because self.model does not have attribute sigma2,
