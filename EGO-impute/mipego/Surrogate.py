...
class RandomForest(RandomForestRegressor):
    ...
    def _check_X(self, X):
        ...
        X_ = atleast_2d(X)
        if hasattr(self, '_levels'): # i.e. self has categorical variable
    ...
    def predict(self, X, eval_MSE=False):
        ...
        if eval_MSE:
            sigma2 = np.std(y_hat_all, axis=1, ddof=1) ** 2. # should sigma2 be self.sigma2 ?
                                    # Because in InfillCriteria EI class there is surrogate_model.sigma2
                                    # However, we don't need self.sigma2, because boolean variable eval_MSE has perfectly solve return value
